...
class MasterAgent:
    ...
    def _register_handlers(self) -> None:
        ...
        # 命令处理
        self.bus.register_command_handler(CommandType.REGISTER, self._handle_register)
        self.bus.register_command_handler(CommandType.UNREGISTER, self._handle_unregister)
    ...
    async def _handle_register(self, message: AgentMessage) -> AgentMessage | None:
        """处理注册请求"""
        agent_info = AgentInfo.from_dict(message.payload)
    ...
    async def _handle_unregister(self, message: AgentMessage) -> AgentMessage | None:
        """处理注销请求"""
    ...
    async def _handle_task_result(self, message: AgentMessage) -> AgentMessage | None:
        """处理任务结果"""
    ...
    async def _handle_chat_response(self, message: AgentMessage) -> AgentMessage | None:
        """处理对话响应"""
    ...
    async def spawn_worker(
        ...
        current_workers = len(
            [
                w
                for w in self.registry.list_by_type(AgentType.WORKER)
